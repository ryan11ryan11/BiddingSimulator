...
FEATURE_VIEWS_SQL = """
CREATE OR REPLACE VIEW v_features_prebid AS
SELECT n.bid_no, n.ord, n.section_date, n.bid_name,
       n.presmpt_prce, n.main_cnstty_nm, n.subsi_cnstty_list,
       n.cntrct_mthd, n.lower_rate_pct,
       n.base_amount, n.range_low_pct, n.range_high_pct, n.base_opened_at,
       l.perms, l.mfrc
FROM t_notice n
LEFT JOIN t_license l USING (bid_no, ord);

-- 라벨(정답) 전용 뷰: 학습 입력에 쓰지 말 것
CREATE OR REPLACE VIEW v_labels AS
SELECT f.bid_no, f.ord,
       f.expected_plnprc, f.lower_rate_pct, f.bid_floor_10won
FROM t_floor f;
"""
...
def ensure_views(engine: Engine):
    with engine.begin() as conn:
        conn.exec_driver_sql(FEATURE_VIEWS_SQL)
...
def is_excluded_cntrct(cntrct_mthd: Optional[str], bad: List[str]) -> bool:
    s = (cntrct_mthd or "")
...
def main(start_date: Optional[str], end_date: Optional[str],
         ...
         scope: str, exclude_cntrct: List[str], create_views: bool):
    global DEBUG
    DEBUG = debug
    ...
    for key, og in og_map.items():
        ...
        # 계약방식 제외 필터
        if is_excluded_cntrct(pb.get("cntrctCnclsMthdNm", ""), exclude_cntrct):
            reason_cntrct += 1
            continue
    ...
    log_info(f"후보 선별: {len(candidates)}건 (금액 탈락 {reason_price}, 키워드/면허 미적중 {reason_kw}, 계약방식 제외 {reason_cntrct})")
...
if __name__ == "__main__":
    ...
    p.add_argument("--scope", choices=["metal","all"], default="all",
                   help="metal=금속/창호/지붕 키워드 필터, all=공사 전체(≤상한)")
    p.add_argument("--exclude-cntrct", nargs="*", default=["단가","수의","유지보수"],
                   help="계약방식 제외 키워드(공백으로 여러 개)")
    p.add_argument("--create-views", action="store_true",
                   help="학습용 사전 특징뷰(v_features_prebid)와 라벨뷰(v_labels)를 생성")
    ...
    main(start_date=sd, end_date=ed, days=args.days,
         ...
         scope=args.scope, exclude_cntrct=args.exclude_cntrct,
